...
def download_model():
    ...
    try:
        ...
        # Khusus untuk Google Drive, tambahkan confirm parameter
        response = requests.get(model_url, timeout=60, stream=True, allow_redirects=True)
        
        # Handle Google Drive virus scan warning untuk file >100MB
        if 'drive.google.com' in model_url and response.status_code == 200:
            content_type = response.headers.get('Content-Type', '')
            if 'text/html' in content_type:
                print("⚠️ Google Drive warning detected, retrying with confirm...")
                # Add confirm parameter
                if '&confirm=' not in model_url:
                    confirm_url = model_url + '&confirm=t'
                    response = requests.get(confirm_url, timeout=60, stream=True, allow_redirects=True)
